...
test("главная строит динамический каталог из 11 карточек", async ({ page }) => {
  const runtimeErrors = collectRuntimeErrors(page);
  await page.goto("/index.html");
  await expect(page.locator("#catalogStatus")).toHaveText("Каталог обновлён.");
  await expect(page.locator("#toolCatalog .tool-card")).toHaveCount(11);
  await expect(page.locator("#categoryNavigation small")).toHaveText([
    "7 тренажёров", "3 тренажёра", "1 инструмент"
  ]);
